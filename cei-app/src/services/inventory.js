...
export const updateItemType = async (itemType) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(`Error al actualizar el tipo de ítem con ID ${itemType.id}: ${errorMessage}`);
    }
...
export const deleteItemType = async (itemTypeID) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      throw new Error(`Error al eliminar el tipo de ítem con ID ${itemTypeID}: ${errorMessage}`);
    }
